...
export default function JobPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AppShell><main className="app-background"><div className="single-column">{job && <>
    <section className="card job-detail"><CompanyLogo name={company?.nombre ?? "Empresa"} src={company?.foto_perfil_url} className="company-placeholder large"/><h1>{job.titulo}</h1>{company && <Link href={`/empresas/${company.id}`}>{company.nombre}</Link>}<p>{company?.industria} · Publicada {formatDate(job.fecha_publicacion)}</p>{applicationAction.membershipMessage ? <p className="standalone-message">{applicationAction.membershipMessage}</p> : applicationAction.showButton && <button onClick={apply} disabled={applicationAction.disabled} className="primary-button">{applicationAction.label}</button>}{message && <p className="standalone-message" role="alert">{message}</p>}</section>
